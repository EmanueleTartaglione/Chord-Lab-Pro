...
/* ── Theory Content ── */
/* Helper: pick string by current language */
function tl(en, it, es, fr) {
  if (state.lang === "it") return it;
  if (state.lang === "es") return es;
  if (state.lang === "fr") return fr;
  return en;
}
...
function getTheorySections() {
  ...
  return [
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "An <strong>interval</strong> is the distance between two notes, measured in semitones (half steps). Intervals are the building blocks of all chords and scales.",
        "Un <strong>intervallo</strong> è la distanza tra due note, misurata in semitoni. Gli intervalli sono i mattoni fondamentali di tutti gli accordi e le scale.",
        "Un <strong>intervalo</strong> es la distancia entre dos notas, medida en semitonos. Los intervalos son los bloques fundamentales de todos los acordes y escalas.",
        "Un <strong>intervalle</strong> est la distance entre deux notes, mesurée en demi-tons. Les intervalles sont les éléments de base de tous les accords et gammes."
      )}</p>
      <p>${tl(
        "Each interval has a unique sound quality — from the tight dissonance of a <strong>minor 2nd</strong> to the perfect consonance of an <strong>octave</strong>.",
        "Ogni intervallo ha una qualità sonora unica — dalla stretta dissonanza di una <strong>seconda minore</strong> alla perfetta consonanza di un'<strong>ottava</strong>.",
        "Cada intervalo tiene una cualidad sonora única — desde la tensa disonancia de una <strong>segunda menor</strong> hasta la perfecta consonancia de una <strong>octava</strong>.",
        "Chaque intervalle a une qualité sonore unique — de la dissonance serrée d'une <strong>seconde mineure</strong> à la consonance parfaite d'une <strong>octave</strong>."
      )}</p>
    </div>
  ...
  },
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "A <strong>triad</strong> is a three-note chord built by stacking thirds. There are four types of triads, each with a distinct emotional quality:",
        "Una <strong>triade</strong> è un accordo di tre note costruito sovrapponendo terze. Ci sono quattro tipi di triadi, ognuna con una qualità emotiva distinta:",
        "Una <strong>tríada</strong> es un acorde de tres notas construido apilando terceras. Hay cuatro tipos de tríadas, cada una con una cualidad emocional distinta:",
        "Une <strong>triade</strong> est un accord de trois notes construit en empilant des tierces. Il existe quatre types de triades, chacune avec une qualité émotionnelle distincte :"
      )}</p>
    </div>
    <table class="interval-table">
      <thead><tr><th>${t("Triad")}</th><th>${t("Formula")}</th><th>${t("Intervals")}</th><th>${t("Sound")}</th><th>${tl("Example","Esempio","Ejemplo","Exemple")}</th><th></th></tr></thead>
      <tbody>
    ...
    <div class="theory-text" style="margin-top:14px;">
      <p>${tl(
        "<strong>How to think about it:</strong> The 3rd determines major vs minor feel. The 5th determines stability (perfect = stable, diminished = tense, augmented = floating).",
        "<strong>Come ragionare:</strong> La 3a determina il carattere maggiore o minore. La 5a determina la stabilità (perfetta = stabile, diminuita = tesa, aumentata = sospesa).",
        "<strong>Cómo pensarlo:</strong> La 3a determina el carácter mayor o menor. La 5a determina la estabilidad (perfecta = estable, disminuida = tensa, aumentada = flotante).",
        "<strong>Comment y penser :</strong> La 3ce détermine le caractère majeur ou mineur. La 5te détermine la stabilité (parfaite = stable, diminuée = tendue, augmentée = flottante)."
      )}</p>
    </div>`
  },
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "<strong>7th chords</strong> add a fourth note — the 7th — on top of a triad. This adds richness and complexity. There are several types:",
        "Gli <strong>accordi di 7a</strong> aggiungono una quarta nota — la 7a — sopra una triade. Questo aggiunge ricchezza e complessità. Ci sono diversi tipi:",
        "Los <strong>acordes de 7a</strong> añaden una cuarta nota — la 7a — sobre una tríada. Esto añade riqueza y complejidad. Hay varios tipos:",
        "Les <strong>accords de 7e</strong> ajoutent une quatrième note — la 7e — au-dessus d'une triade. Cela ajoute richesse et complexité. Il en existe plusieurs types :"
      )}</p>
    </div>
    <table class="interval-table">
      <thead><tr><th>${t("7th Chords")}</th><th>${t("Formula")}</th><th>${tl("Built From","Costruito da","Construido de","Construit de")}</th><th>${t("Sound Quality")}</th><th></th></tr></thead>
      <tbody>
        <tr><td><strong>${tl("Dominant 7th","Settima dominante","Séptima dominante","Septième de dominante")}</strong> (C7)</td><td>1–3–5–b7</td><td>${tl("Major triad + m7","Triade magg. + m7","Tríada mayor + m7","Triade maj. + m7")}</td><td>${tl("Bluesy tension, wants to resolve. The V chord.","Tensione blues, vuole risolvere. Il V grado.","Tensión blues, quiere resolver. El V grado.","Tension blues, veut résoudre. Le V degré.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,10">${p}</button></td></tr>
        <tr><td><strong>${tl("Major 7th","Settima maggiore","Séptima mayor","Septième majeure")}</strong> (Cmaj7)</td><td>1–3–5–7</td><td>${tl("Major triad + M7","Triade magg. + M7","Tríada mayor + M7","Triade maj. + M7")}</td><td>${tl("Lush, dreamy. Jazz ballads, bossa nova.","Lussureggiante, sognante. Ballad jazz, bossa nova.","Exuberante, soñador. Baladas jazz, bossa nova.","Luxuriant, rêveur. Ballades jazz, bossa nova.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,11">${p}</button></td></tr>
        <tr><td><strong>${tl("Minor 7th","Settima minore","Séptima menor","Septième mineure")}</strong> (Cm7)</td><td>1–b3–5–b7</td><td>${tl("Minor triad + m7","Triade min. + m7","Tríada menor + m7","Triade min. + m7")}</td><td>${tl("Smooth, mellow. The ii chord in jazz.","Morbido, dolce. L'accordo di ii nel jazz.","Suave, meloso. El ii grado en jazz.","Doux, moelleux. Le ii degré en jazz.")}</td><td><button class="play-interval-btn" data-chord="0,3,7,10">${p}</button></td></tr>
        <tr><td><strong>${tl("Diminished 7th","Settima diminuita","Séptima disminuida","Septième diminuée")}</strong> (Cdim7)</td><td>1–b3–b5–bb7</td><td>${tl("Dim triad + dim7","Triade dim + dim7","Tríada dim + dim7","Triade dim + dim7")}</td><td>${tl("Maximum tension, symmetric. Classical drama.","Tensione massima. Dramma classico.","Tensión máxima. Drama clásico.","Tension maximale. Drame classique.")}</td><td><button class="play-interval-btn" data-chord="0,3,6,9">${p}</button></td></tr>
        <tr><td><strong>${tl("Half-diminished","Semidiminuito","Semidisminuido","Semi-diminué")}</strong> (Cm7b5)</td><td>1–b3–b5–b7</td><td>${tl("Dim triad + m7","Triade dim + m7","Tríada dim + m7","Triade dim + m7")}</td><td>${tl("Dark tension. ii in minor-key jazz.","Tensione scura. ii nel jazz in minore.","Tensión oscura. ii en jazz menor.","Tension sombre. Le ii en jazz mineur.")}</td><td><button class="play-interval-btn" data-chord="0,3,6,10">${p}</button></td></tr>
        <tr><td><strong>${tl("Minor-Major 7th","Minore-Magg. 7a","Menor-Mayor 7a","Mineure-Majeure 7e")}</strong> (Cm(maj7))</td><td>1–b3–5–7</td><td>${tl("Minor triad + M7","Triade min. + M7","Tríada menor + M7","Triade min. + M7")}</td><td>${tl("Mysterious, noir. James Bond chord.","Misterioso, noir. L'accordo di James Bond.","Misterioso, noir. El acorde de James Bond.","Mystérieux, noir. L'accord de James Bond.")}</td><td><button class="play-interval-btn" data-chord="0,3,7,11">${p}</button></td></tr>
      </tbody>
    </table>
    <div class="theory-text" style="margin-top:14px;">
      <p>${tl(
        "<strong>Key insight:</strong> Dominant 7 = major triad + minor 7th — this mismatch is what creates the 'need to resolve'.",
        "<strong>Intuizione chiave:</strong> 7a dominante = triade maggiore + 7a minore — questa discrepanza crea il 'bisogno di risolvere'.",
        "<strong>Intuición clave:</strong> 7a dominante = tríada mayor + 7a menor — esta discrepancia crea la 'necesidad de resolver'.",
        "<strong>Intuition clé :</strong> 7e dominante = triade majeure + 7e mineure — ce décalage crée le 'besoin de résoudre'."
      )}</p>
    </div>`
  },
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "<strong>Suspended chords</strong> replace the 3rd with either the 2nd or 4th. Since the 3rd defines major/minor, removing it creates an ambiguous, open sound.",
        "Gli <strong>accordi sospesi</strong> sostituiscono la 3a con la 2a o la 4a. Poiché la 3a definisce maggiore/minore, rimuoverla crea un suono ambiguo e aperto.",
        "Los <strong>acordes suspendidos</strong> reemplazan la 3a con la 2a o la 4a. Como la 3a define mayor/menor, eliminarla crea un sonido ambiguo y abierto.",
        "Les <strong>accords suspendus</strong> remplacent la 3ce par la 2de ou la 4te. La 3ce définissant majeur/mineur, la retirer crée un son ambigu et ouvert."
      )}</p>
    </div>
    <table class="interval-table">
      <thead><tr><th>${tl("Chord","Accordo","Acorde","Accord")}</th><th>${t("Formula")}</th><th>${tl("What Happens","Cosa succede","Qué sucede","Ce qui se passe")}</th><th>${t("Sound")}</th><th></th></tr></thead>
      <tbody>
        <tr><td><strong>sus2</strong> (Csus2)</td><td>1–2–5</td><td>${tl("3rd replaced by major 2nd","3a sostituita dalla 2a magg.","3a reemplazada por 2a mayor","3ce remplacée par 2de maj.")}</td><td>${tl("Open, modern, airy. U2, The Police.","Aperto, moderno. U2, The Police.","Abierto, moderno. U2, The Police.","Ouvert, moderne. U2, The Police.")}</td><td><button class="play-interval-btn" data-chord="0,2,7">${p}</button></td></tr>
        <tr><td><strong>sus4</strong> (Csus4)</td><td>1–4–5</td><td>${tl("3rd replaced by perfect 4th","3a sostituita dalla 4a perf.","3a reemplazada por 4a perf.","3ce remplacée par 4te perf.")}</td><td>${tl("Yearning, unresolved. Wants to fall to major 3rd.","Bramoso, irrisolto. Vuole cadere sulla 3a.","Anhelante. Quiere caer a la 3a mayor.","Nostalgique. Veut tomber sur la 3ce.")}</td><td><button class="play-interval-btn" data-chord="0,5,7">${p}</button></td></tr>
        <tr><td><strong>7sus4</strong> (C7sus4)</td><td>1–4–5–b7</td><td>${tl("Dominant + suspended 4th","Dominante + 4a sospesa","Dominante + 4a suspendida","Dominante + 4te suspendue")}</td><td>${tl("Floating tension. Common before V7 resolution.","Tensione fluttuante. Prima della risoluzione V7.","Tensión flotante. Antes de la resolución V7.","Tension flottante. Avant la résolution V7.")}</td><td><button class="play-interval-btn" data-chord="0,5,7,10">${p}</button></td></tr>
      </tbody>
    </table>
    <div class="theory-text" style="margin-top:14px;">
      <p>${tl(
        "<strong>Tip:</strong> sus4 → major is one of the most satisfying resolutions in music.",
        "<strong>Suggerimento:</strong> sus4 → maggiore è una delle risoluzioni più soddisfacenti nella musica.",
        "<strong>Consejo:</strong> sus4 → mayor es una de las resoluciones más satisfactorias en la música.",
        "<strong>Astuce :</strong> sus4 → majeur est l'une des résolutions les plus satisfaisantes en musique."
      )}</p>
    </div>`
  },
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "<strong>Added tone chords</strong> take a basic triad and add one extra note <em>without</em> including the 7th. This distinguishes them from extended chords (9, 11, 13).",
        "Gli <strong>accordi con note aggiunte</strong> prendono una triade e aggiungono una nota <em>senza</em> includere la 7a. Questo li distingue dagli accordi estesi (9, 11, 13).",
        "Los <strong>acordes con notas añadidas</strong> toman una tríada y agregan una nota extra <em>sin</em> incluir la 7a. Los distingue de los acordes extendidos (9, 11, 13).",
        "Les <strong>accords avec notes ajoutées</strong> prennent une triade et ajoutent une note <em>sans</em> inclure la 7e. Ils se distinguent des accords étendus (9, 11, 13)."
      )}</p>
    </div>
    <table class="interval-table">
      <thead><tr><th>${tl("Chord","Accordo","Acorde","Accord")}</th><th>${t("Formula")}</th><th>${tl("What's Added","Cosa si aggiunge","Qué se añade","Ce qui est ajouté")}</th><th>${t("Sound")}</th><th></th></tr></thead>
      <tbody>
        <tr><td><strong>add9</strong> (Cadd9)</td><td>1–2–3–5</td><td>${tl("Major 9th (2nd up an octave)","9a maggiore (2a un'ottava sopra)","9a mayor (2a una octava arriba)","9e majeure (2de une octave plus haut)")}</td><td>${tl("Bright sparkle. Pop staple.","Scintillio brillante. Classico del pop.","Brillo brillante. Clásico del pop.","Éclat brillant. Classique de la pop.")}</td><td><button class="play-interval-btn" data-chord="0,2,4,7">${p}</button></td></tr>
        <tr><td><strong>madd9</strong> (Cm(add9))</td><td>1–2–b3–5</td><td>${tl("Major 9th to minor triad","9a maggiore alla triade minore","9a mayor a la tríada menor","9e majeure à la triade mineure")}</td><td>${tl("Moody, atmospheric.","Umorale, atmosferico.","Melancólico, atmosférico.","Mélancolique, atmosphérique.")}</td><td><button class="play-interval-btn" data-chord="0,2,3,7">${p}</button></td></tr>
        <tr><td><strong>add11</strong> (Cadd11)</td><td>1–3–4–5</td><td>${tl("Perfect 11th (= 4th)","11a perfetta (= 4a)","11a perfecta (= 4a)","11e parfaite (= 4te)")}</td><td>${tl("Tension with the 3rd. Use carefully.","Tensione con la 3a. Usare con cautela.","Tensión con la 3a. Usar con cuidado.","Tension avec la 3ce. À utiliser avec précaution.")}</td><td><button class="play-interval-btn" data-chord="0,4,5,7">${p}</button></td></tr>
        <tr><td><strong>add13</strong> (Cadd13)</td><td>1–3–5–6</td><td>${tl("Major 13th (= 6th)","13a maggiore (= 6a)","13a mayor (= 6a)","13e majeure (= 6te)")}</td><td>${tl("Same as a 6th chord. Context determines name.","Uguale a un accordo di 6a.","Igual que un acorde de 6a.","Identique à un accord de 6te.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,9">${p}</button></td></tr>
      </tbody>
    </table>
    <div class="theory-text" style="margin-top:14px;">
      <p>${tl(
        "<strong>add9 vs 9:</strong> Cadd9 = C E G D (no 7th). C9 = C E G Bb D (has the 7th). The add9 is simpler and more pop-friendly.",
        "<strong>add9 vs 9:</strong> Cadd9 = Do Mi Sol Re (senza 7a). C9 = Do Mi Sol Sib Re (con la 7a). L'add9 è più semplice e pop.",
        "<strong>add9 vs 9:</strong> Cadd9 = Do Mi Sol Re (sin 7a). C9 = Do Mi Sol Sib Re (con la 7a). El add9 es más simple y pop.",
        "<strong>add9 vs 9 :</strong> Cadd9 = Do Mi Sol Ré (sans 7e). C9 = Do Mi Sol Sib Ré (avec la 7e). Le add9 est plus simple et pop."
      )}</p>
    </div>`
  },
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "<strong>Extended chords</strong> stack thirds beyond the 7th: 9th, 11th, 13th. Each implies all lower extensions are present.",
        "Gli <strong>accordi estesi</strong> sovrappongono terze oltre la 7a: 9a, 11a, 13a. Ognuno implica che tutte le estensioni inferiori siano presenti.",
        "Los <strong>acordes extendidos</strong> apilan terceras más allá de la 7a: 9a, 11a, 13a. Cada uno implica que todas las extensiones inferiores están presentes.",
        "Les <strong>accords étendus</strong> empilent des tierces au-delà de la 7e : 9e, 11e, 13e. Chacun implique que toutes les extensions inférieures sont présentes."
      )}</p>
    </div>
    <table class="interval-table">
      <thead><tr><th>${tl("Chord","Accordo","Acorde","Accord")}</th><th>${t("Formula")}</th><th>${tl("Key Interval","Intervallo chiave","Intervalo clave","Intervalle clé")}</th><th>${t("Sound Quality")}</th><th></th></tr></thead>
      <tbody>
        <tr><td><strong>9</strong> (C9)</td><td>1–3–5–b7–9</td><td>Dom7 + M9</td><td>${tl("Funky, soulful. James Brown, Stevie Wonder.","Funky, pieno di soul.","Funky, con alma.","Funky, plein de soul.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,10,14">${p}</button></td></tr>
        <tr><td><strong>maj9</strong> (Cmaj9)</td><td>1–3–5–7–9</td><td>Maj7 + M9</td><td>${tl("Beautiful, lush. Neo-soul, dream pop.","Bello, lussureggiante. Neo-soul, dream pop.","Hermoso, exuberante. Neo-soul, dream pop.","Beau, luxuriant. Neo-soul, dream pop.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,11,14">${p}</button></td></tr>
        <tr><td><strong>m9</strong> (Cm9)</td><td>1–b3–5–b7–9</td><td>Min7 + M9</td><td>${tl("Smooth, sophisticated. Contemporary R&B.","Morbido, sofisticato. R&B contemporaneo.","Suave, sofisticado. R&B contemporáneo.","Doux, sophistiqué. R&B contemporain.")}</td><td><button class="play-interval-btn" data-chord="0,3,7,10,14">${p}</button></td></tr>
        <tr><td><strong>11</strong> (C11)</td><td>1–3–5–b7–9–11</td><td>Dom9 + P11</td><td>${tl("Wide, open. Often omit the 3rd to avoid clash.","Ampio, aperto. Spesso si omette la 3a.","Amplio, abierto. A menudo se omite la 3a.","Large, ouvert. On omet souvent la 3ce.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,10,14,17">${p}</button></td></tr>
        <tr><td><strong>m11</strong> (Cm11)</td><td>1–b3–5–b7–9–11</td><td>Min9 + P11</td><td>${tl("Rich, deep. D'Angelo, Hiatus Kaiyote.","Ricco, profondo.","Rico, profundo.","Riche, profond.")}</td><td><button class="play-interval-btn" data-chord="0,3,7,10,14,17">${p}</button></td></tr>
        <tr><td><strong>13</strong> (C13)</td><td>1–3–5–b7–9–13</td><td>Dom9 + M13</td><td>${tl("Full, shimmering. Jazz big band, funk horns.","Pieno, scintillante. Big band, fiati funk.","Lleno, brillante. Big band, metales funk.","Plein, chatoyant. Big band jazz, cuivres funk.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,10,14,21">${p}</button></td></tr>
        <tr><td><strong>maj13</strong> (Cmaj13)</td><td>1–3–5–7–9–13</td><td>Maj9 + M13</td><td>${tl("The ultimate lush chord. Orchestral jazz.","L'accordo lussureggiante per eccellenza.","El acorde exuberante definitivo.","L'accord luxuriant ultime.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,11,14,21">${p}</button></td></tr>
      </tbody>
    </table>`
  },
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "<strong>Altered chords</strong> modify (raise or lower) one or more notes of a standard chord to create extra tension or color. Most common on dominant 7th chords.",
        "Gli <strong>accordi alterati</strong> modificano (alzano o abbassano) note di un accordo standard per creare tensione extra. Più comuni sugli accordi di 7a dominante.",
        "Los <strong>acordes alterados</strong> modifican (suben o bajan) notas de un acorde estándar para crear tensión extra. Más comunes en acordes de 7a dominante.",
        "Les <strong>accords altérés</strong> modifient (haussent ou baissent) des notes d'un accord standard pour créer une tension supplémentaire. Plus courants sur les 7e de dominante."
      )}</p>
    </div>
    <table class="interval-table">
      <thead><tr><th>${tl("Alteration","Alterazione","Alteración","Altération")}</th><th>${tl("What It Does","Cosa fa","Qué hace","Ce que ça fait")}</th><th>${tl("Example Chord","Accordo esempio","Acorde ejemplo","Accord exemple")}</th><th>${t("Sound Quality")}</th><th></th></tr></thead>
      <tbody>
        <tr><td><strong>b5</strong></td><td>${tl("Lowers the 5th by a half step","Abbassa la 5a di un semitono","Baja la 5a medio tono","Abaisse la 5te d'un demi-ton")}</td><td>C7b5: C E Gb Bb</td><td>${tl("Tritone sub. Chromatic bass movement.","Sostituzione del tritono. Basso cromatico.","Sustitución del tritono. Bajo cromático.","Substitution tritonique. Basse chromatique.")}</td><td><button class="play-interval-btn" data-chord="0,4,6,10">${p}</button></td></tr>
        <tr><td><strong>#5</strong></td><td>${tl("Raises the 5th by a half step","Alza la 5a di un semitono","Sube la 5a medio tono","Hausse la 5te d'un demi-ton")}</td><td>C7#5: C E G# Bb</td><td>${tl("Augmented dominant. Strong pull to resolution.","Dominante aumentata. Forte attrazione.","Dominante aumentada. Fuerte atracción.","Dominante augmentée. Forte attraction.")}</td><td><button class="play-interval-btn" data-chord="0,4,8,10">${p}</button></td></tr>
        <tr><td><strong>b9</strong></td><td>${tl("Lowers the 9th by a half step","Abbassa la 9a di un semitono","Baja la 9a medio tono","Abaisse la 9e d'un demi-ton")}</td><td>C7b9: C E G Bb Db</td><td>${tl("Dark, Spanish flavor. Flamenco, minor key jazz.","Scuro, sapore spagnolo. Flamenco, jazz in minore.","Oscuro, sabor español. Flamenco, jazz menor.","Sombre, saveur espagnole. Flamenco, jazz mineur.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,10,13">${p}</button></td></tr>
        <tr><td><strong>#9</strong></td><td>${tl("Raises the 9th by a half step","Alza la 9a di un semitono","Sube la 9a medio tono","Hausse la 9e d'un demi-ton")}</td><td>C7#9: C E G Bb D#</td><td>${tl("The Hendrix chord. Gritty, bluesy.","L'accordo di Hendrix. Ruvido, blues.","El acorde de Hendrix. Crudo, blusero.","L'accord de Hendrix. Brut, blues.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,10,15">${p}</button></td></tr>
        <tr><td><strong>#11</strong></td><td>${tl("Raises the 11th by a half step","Alza l'11a di un semitono","Sube la 11a medio tono","Hausse la 11e d'un demi-ton")}</td><td>Cmaj7#11: C E F# G B</td><td>${tl("Lydian sound. Bright, floating, cinematic.","Suono lidio. Brillante, fluttuante.","Sonido lidio. Brillante, flotante.","Son lydien. Brillant, flottant.")}</td><td><button class="play-interval-btn" data-chord="0,4,6,7,11">${p}</button></td></tr>
        <tr><td><strong>b13</strong></td><td>${tl("Lowers the 13th by a half step","Abbassa la 13a di un semitono","Baja la 13a medio tono","Abaisse la 13e d'un demi-ton")}</td><td>C7b13: C E G Ab Bb</td><td>${tl("Augmented color in a dominant context.","Colore aumentato in un contesto dominante.","Color aumentado en contexto dominante.","Couleur augmentée en contexte de dominante.")}</td><td><button class="play-interval-btn" data-chord="0,4,7,8,10">${p}</button></td></tr>
      </tbody>
    </table>
    <div class="theory-text" style="margin-top:14px;">
      <p>${tl(
        "The <strong>altered scale</strong> (Super Locrian) contains ALL alterations: b9, #9, b5, #5. It's the ultimate tension scale over a V7 chord.",
        "La <strong>scala alterata</strong> (Super Locria) contiene TUTTE le alterazioni: b9, #9, b5, #5. È la scala di tensione definitiva sopra un V7.",
        "La <strong>escala alterada</strong> (Super Locria) contiene TODAS las alteraciones: b9, #9, b5, #5. Es la escala de tensión definitiva sobre un V7.",
        "La <strong>gamme altérée</strong> (Super Locrienne) contient TOUTES les altérations : b9, #9, b5, #5. C'est la gamme de tension ultime sur un V7."
      )}</p>
    </div>`
  },
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "A <strong>slash chord</strong> (e.g., C/E) means play C chord with E in the bass. When the bass note is a chord tone, it's called an <strong>inversion</strong>.",
        "Un <strong>accordo con basso</strong> (es. C/E) significa suona l'accordo di C con E al basso. Quando la nota al basso è una nota dell'accordo, si chiama <strong>rivolto</strong>.",
        "Un <strong>acorde con bajo</strong> (ej. C/E) significa toca el acorde de C con E en el bajo. Cuando la nota del bajo es una nota del acorde, se llama <strong>inversión</strong>.",
        "Un <strong>accord avec basse</strong> (ex. C/E) signifie jouer l'accord de C avec E à la basse. Quand la note de basse est une note de l'accord, on l'appelle un <strong>renversement</strong>."
      )}</p>
    </div>
    <table class="interval-table">
      <thead><tr><th>${tl("Position","Posizione","Posición","Position")}</th><th>${tl("Bass Note","Nota al basso","Nota del bajo","Note de basse")}</th><th>${tl("Example","Esempio","Ejemplo","Exemple")}</th><th>${t("Sound")}</th><th></th></tr></thead>
      <tbody>
        <tr><td><strong>${tl("Root position","Stato fondamentale","Estado fundamental","État fondamental")}</strong></td><td>${tl("Root in bass","Fondamentale al basso","Fundamental en el bajo","Fondamentale à la basse")}</td><td>C/C: C E G</td><td>${tl("Most stable, grounded.","Più stabile, radicato.","Más estable, sólido.","Le plus stable, ancré.")}</td><td><button class="play-interval-btn" data-chord="0,4,7">${p}</button></td></tr>
        <tr><td><strong>${tl("1st inversion","1° rivolto","1ª inversión","1er renversement")}</strong></td><td>${tl("3rd in bass","3a al basso","3a en el bajo","3ce à la basse")}</td><td>C/E: E G C</td><td>${tl("Lighter, flowing. Great for bass lines.","Più leggero, fluido. Ottimo per linee di basso.","Más ligero, fluido. Excelente para líneas de bajo.","Plus léger, fluide. Excellent pour les lignes de basse.")}</td><td><button class="play-interval-btn" data-chord="-8,0,4,7">${p}</button></td></tr>
        <tr><td><strong>${tl("2nd inversion","2° rivolto","2ª inversión","2e renversement")}</strong></td><td>${tl("5th in bass","5a al basso","5a en el bajo","5te à la basse")}</td><td>C/G: G C E</td><td>${tl("Open, less stable. Classical cadential 6/4.","Aperto, meno stabile. Cadenza classica 6/4.","Abierto, menos estable. Cadencia clásica 6/4.","Ouvert, moins stable. Cadence classique 6/4.")}</td><td><button class="play-interval-btn" data-chord="-5,0,4,7">${p}</button></td></tr>
        <tr><td><strong>${tl("3rd inversion","3° rivolto","3ª inversión","3e renversement")}</strong> (7th)</td><td>${tl("7th in bass","7a al basso","7a en el bajo","7e à la basse")}</td><td>C7/Bb: Bb C E G</td><td>${tl("Smooth voice leading. Descending bass lines.","Conduzione delle voci fluida. Basso discendente.","Conducción de voces suave. Bajo descendente.","Conduite des voix fluide. Basse descendante.")}</td><td><button class="play-interval-btn" data-chord="-2,0,4,7,10">${p}</button></td></tr>
      </tbody>
    </table>
    <div class="theory-text" style="margin-top:14px;">
      <p>${tl(
        "<strong>Practical use:</strong> Inversions create smooth, stepwise bass lines. Try: C → C/E → F → F/A → G/B → C. The bass walks up: C-E-F-A-B-C.",
        "<strong>Uso pratico:</strong> I rivolti creano linee di basso fluide. Prova: C → C/E → F → F/A → G/B → C. Il basso sale: Do-Mi-Fa-La-Si-Do.",
        "<strong>Uso práctico:</strong> Las inversiones crean líneas de bajo suaves. Prueba: C → C/E → F → F/A → G/B → C. El bajo sube: Do-Mi-Fa-La-Si-Do.",
        "<strong>Utilisation pratique :</strong> Les renversements créent des lignes de basse fluides. Essayez : C → C/E → F → F/A → G/B → C."
      )}</p>
    </div>`
  },
  {
    ...
    content: `<div class="theory-text">
      <p>${tl(
        "A <strong>power chord</strong> is just the root + perfect 5th (often doubled at the octave). It contains NO 3rd, making it neither major nor minor.",
        "Un <strong>power chord</strong> è semplicemente la fondamentale + la 5a perfetta. Non contiene la 3a, quindi non è né maggiore né minore.",
        "Un <strong>power chord</strong> es simplemente la fundamental + la 5a perfecta. No contiene la 3a, por lo que no es ni mayor ni menor.",
        "Un <strong>power chord</strong> est simplement la fondamentale + la 5te parfaite. Il ne contient PAS de 3ce, ce qui le rend ni majeur ni mineur."
      )}</p>
    </div>
    <table class="interval-table">
      <thead><tr><th>${tl("Chord","Accordo","Acorde","Accord")}</th><th>${t("Formula")}</th><th>${t("Notes")}</th><th></th></tr></thead>
      <tbody>
        <tr><td><strong>C5</strong></td><td>1 – 5</td><td>C G</td><td><button class="play-interval-btn" data-chord="0,7">${p}</button></td></tr>
        <tr><td><strong>C5 (${tl("octave","ottava","octava","octave")})</strong></td><td>1 – 5 – 8</td><td>C G C</td><td><button class="play-interval-btn" data-chord="0,7,12">${p}</button></td></tr>
      </tbody>
    </table>
    <div class="theory-text" style="margin-top:14px;">
      <p>${tl(
        "<strong>Why power chords work with distortion:</strong> Distortion adds harmonics. The perfect 5th's overtones reinforce each other cleanly — a 3rd would create harsh overtones.",
        "<strong>Perché funzionano con la distorsione:</strong> La distorsione aggiunge armoniche. Le armoniche della 5a perfetta si rinforzano pulitamente — una 3a creerebbe armoniche aspre.",
        "<strong>Por qué funcionan con distorsión:</strong> La distorsión añade armónicos. Los de la 5a perfecta se refuerzan limpiamente — una 3a crearía armónicos ásperos.",
        "<strong>Pourquoi ils fonctionnent avec la distorsion :</strong> La distorsion ajoute des harmoniques. Ceux de la 5te parfaite se renforcent proprement — une 3ce créerait des harmoniques dures."
      )}</p>
      <p><strong>${tl("Common power chord progressions:","Progressioni comuni:","Progresiones comunes:","Progressions courantes :")}</strong></p>
